...
from django.contrib.auth.models import User  #导入User表，用于项目的管理用户表
...
class Asset(models.Model):
    ...
    business_unit = models.ForeignKey('BusinessUnit', null=True,
                                      blank=True, verbose_name='所属业务线', on_delete=models.CASCADE)
    ...
    purchase_day = models.DateTimeField(null=True, blank=True, verbose_name='购买日期')
    ...
    approved_by = models.ForeignKey(User, null=True, blank=True, related_name='approved_by', verbose_name='批准人', on_delete=models.CASCADE)

    memo = models.TimeField(null=True, blank=True, verbose_name='备注')
...
class Server(models.Model):
    ...
    hosted_on = models.ForeignKey('self', related_name='hosted_on_server',
                                  blank=True, null=True, verbose_name='服务器型号', on_delete=models.CASCADE)
...
class StorageDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, default=0, verbose_name='储存设备类型')
...
class NetworkDevice(models.Model):
    ...
    device_detail = models.TimeField(null=True, blank=True, verbose_name='详细配置')
...
class Software(models.Model):
        ...
        version = models.CharField(max_length=64, unique=True, help_text='例如：Cents release 6.7 （Final）',
...
class Manufacturer(models.Model):
    """ 产商 """
    name = models.CharField('产商名称', max_length=64, unique=True)
...
class BusinessUnit(models.Model):
    ...
    parent_unit = models.ForeignKey('self', blank=True, null=True,
                                    related_name='parent_level', on_delete=models.CASCADE)
...
class Contract(models.Model):
    ...
    star_day = models.DateTimeField('开始日期', blank=True, null=True)
    end_day = models.DateTimeField('失效日期', blank=True, null=True)
    license_num = models.IntegerField('license数量', blank=True, null=True)
    c_day = models.DateTimeField('创建日期', auto_now_add=True)
    m_day = models.DateTimeField('修改日期', auto_now=True)
...
class Disk(models.Model):
    ...
    slot = models.CharField('所在插槽', max_length=64, blank=True, null=True)
    ...
    def __str__(self):
        return '%s:  %s:  %sGB' % (self.asset.name, self.model, self.capacity)
...
class NIC(models.Model):
    ...
    mac = models.CharField('MAC地址', max_length=64)  # 虚拟机有可能出现同样的mac地址
...
class EventLog(models.Model):
    ...
    因此，删除字段，on_delete = models.SET_NULL
    """
